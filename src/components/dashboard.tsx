    ...
    <div className="space-y-6 fade-in">
      <Card>
        <CardHeader className="flex flex-row items-center justify-between pb-2">
          <CardTitle className="text-sm font-medium">FIXIE TOKEN</CardTitle>
          <span className="text-sm font-medium text-primary">$FIXIE</span>
        </CardHeader>
        <CardContent>
      ...
      <Card>
        ...
        <CardContent>
            <div className="w-full h-64 rounded-lg mb-4 relative overflow-hidden bg-secondary/20">
                <Image
            ...
            <Button className="w-full">
                <Dumbbell className="mr-2 h-4 w-4" /> Start Ride
            </Button>
        </CardContent>
      </Card>
...
const StatCard = ({ icon, label, value, progress }: { icon: React.ReactNode, label: string, value: string, progress?: number }) => (
    <Card className="flex flex-col items-center justify-center p-4 bg-secondary/50 border-border/50">
        {icon}
